...
export class DistributorService {
  ...
  /** Updates the limit and stores the difference from the calculated debt. */
  async updateFinancials(id: number, creditLimit: number, currentDebt: number, previousDebt: number): Promise<string | null> {
    const { data: distributor, error: readError } = await supabase
      .from('distributors')
      .select('debt_adjustment')
      .eq('id', id)
      .single();
    if (readError || !distributor) return readError?.message ?? 'Дистрибьютор табылмады';

    const difference = Math.round((currentDebt - previousDebt) * 100) / 100;
    const debtAdjustment = Number(distributor.debt_adjustment ?? 0) + difference;
    const { error } = await supabase
      .from('distributors')
      .update({ credit_limit: creditLimit, debt_adjustment: debtAdjustment })
      .eq('id', id);
    if (error) return error.message;
